...
public class ShoppingItemAdapter extends RecyclerView.Adapter<ShoppingItemAdapter.VH> {
    ...
    public void onBindViewHolder(@NonNull VH h, int position) {
        ...
        // ✅ 方案1：如果是替换品，直接显示灰色打勾（不依赖conflictMap）
        if (it.isSubstituted) {
            ...
            h.conflictBadge.setOnClickListener(v -> {
                String message = it.substituteDisplayName != null ?
                        "✓ " + it.substituteDisplayName :
                        "✓ Replaced item";
            ...
            return;  // ← 直接返回，不再检查conflictMap
        }

        // ✅ 方案2：不是替换品，检查conflictMap
